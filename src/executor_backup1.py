...
class ExecutionEngine:
    ...
    def _determine_task_type_and_confidence(self, task_prompt: Dict[str, Any]) -> Tuple[str, str]:
        ...
        try:
            ...
            analysis_text = ""
            if 'components' in task_prompt:
                components = task_prompt['components']
                analysis_text = f"{components.get('task', '')} {components.get('purpose', '')} {components.get('context', '')}"
            elif 'description' in task_prompt:
            ...
            try:
                ...
                logger.debug(f"Task analysis result: {analysis_data}")
                
            except Exception as e:
                logger.warning(f"Meta-generation task analysis failed: {e}, using heuristic approach")
                task_type, confidence_level = self._heuristic_task_analysis(analysis_text)
            ...
            if task_type not in valid_task_types:
                logger.warning(f"Invalid task type '{task_type}', defaulting to 'deduction'")
                task_type = 'deduction'
            
            if confidence_level not in valid_confidence_levels:
                logger.warning(f"Invalid confidence level '{confidence_level}', defaulting to 'medium'")
