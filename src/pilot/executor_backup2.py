...
class MemoryManager:
    """
    Manages memory operations according to memory_db_schema.json.
    Handles short-term task memory, short-term goal memory, procedural memory, and meta memory.
    """
...
class KnowledgeBaseManager:
    """
    Manages knowledge base creation and access using vector and graph databases.
    """
    
    def __init__(self, config: Dict[str, Any]):
        """Initialize KnowledgeBaseManager with configuration."""
        self.config = config
        self.datautility = DataUtility()
...
class ContextManager:
    """
    Manages context identification, retrieval, and storage.
    """
    ...
    def __init__(self, 
        ...
        """Initialize ContextManager with database paths and configuration."""
        ...
        self.vector_processor = VectorDBRetrievalProcessor(
            vector_file=vector_db_path,
            generator=self.generator
        )
        self.graph_processor = GraphDBRetrievalProcessor(
            graph_file=graph_db_path
        )
    ...
    def retrieve_short_term_goal_memory(self, common_contexts: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            for i, context in enumerate(common_contexts):
                ...
                memory_id = self.memory_manager.store_goal_memory(
    ...
    def assess_context_sufficiency(self, task_prompt: Dict[str, Any], task_id: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            available_contexts = self.memory_manager.get_goal_memory()
            task_contexts = self.memory_manager.get_task_memory(task_id)
            ...
            sufficiency_assessment = self.metagenerator.get_meta_generation(
                ...
                action="disambiguate",
                prompt_id=1001,
                task_prompt=f"""Task requirements: {task_requirements['context_description']}
...
Assess if the available context is sufficient to execute the task effectively. If not, specify what additional information is needed.""",
    ...
    def retrieve_task_specific_context(self, task_id: str, query: str) -> str:
        ...
        try:
            ...
            memory_id = self.memory_manager.store_task_memory(
                task_id=task_id,
...
class ExecutionEngine:
    ...
    def __init__(self, config: Dict[str, Any], memory_manager: MemoryManager):
        """Initialize ExecutionEngine with configuration and memory manager."""
        self.config = config
        self.memory_manager = memory_manager
    ...
    def _determine_task_type_and_confidence(self, task_prompt: Dict[str, Any]) -> Tuple[str, str]:
        ...
        try:
            ...
            # Use meta-generation to analyze task
            try:
                analysis_response = self.metagenerator.get_meta_generation(
                    application="metaprompt",
                    category="evaluation",
                    action="disambiguate",
                    prompt_id=1002, # Ensure this prompt_id is defined in meta_prompt_library.json
                    ...
                    model=self.mle_config.get("defaults", {}).get("metaprompt_model", "Qwen2.5-1.5B"), # Use a configurable model
                ...
                logger.warning(f"Meta-generation task analysis failed: {e_meta}, using heuristic approach on: '{analysis_text[:100]}...'" )
                # If this metagenerator call fails, it should fall back to the *other* heuristic method
                # which we will modify later to be LLM-based.
                # For now, this specific instance of _determine_task_type_and_confidence is distinct
                # from the one that *contains* _heuristic_task_analysis as its primary fallback.
                # This one is *called by* execute_task, the other one is *called by* orchestrate_execution_flow implicitly.
                # So, its fallback should be simple or call the *other* heuristic if that's intended design.
                # Let's assume for now if *this* fails, it uses its own simple heuristic, not the one defined later.
                # This might be an area for future refinement on how fallbacks are chained.
                logger.warning(f"Meta-generation failed in the primary _determine_task_type_and_confidence. Applying simple fallback.")
                if 'complex' in analysis_text.lower(): confidence_level = 'high'
                elif 'simple' in analysis_text.lower(): confidence_level = 'low'
                else: confidence_level = 'medium'
                task_type = 'deduction' # simple default

            # Validate results for this specific method's expectations
            valid_task_types = ['deduction', 'documentation', 'classification', 'clustering', 'induction', 'entity_detection']
            valid_confidence_levels = ['high', 'medium', 'low']
            
            if task_type not in valid_task_types:
                logger.warning(f"Invalid task type '{task_type}' from analysis, defaulting to 'deduction'. Valid: {valid_task_types}")
                task_type = 'deduction'
            
            if confidence_level not in valid_confidence_levels:
                logger.warning(f"Invalid confidence level '{confidence_level}' from analysis, defaulting to 'medium'. Valid: {valid_confidence_levels}")
                confidence_level = 'medium'
            
            return task_type, confidence_level
            
        except Exception as e_main:
            logger.error(f"Overall task type and confidence determination failed in primary method: {e_main}\n{traceback.format_exc()}")
            logger.warning("Defaulting to 'deduction' and 'medium' due to main exception in primary method.")
    ...
    def _execute_with_topology(self,
        ...
        try:
            ...
            if topology == "default":
                ...
                # Topology 12: Regenerative Majority Synthesis
        ...
        for i, task_definition in enumerate(plan):
            ...
            try:
                ...
                if execution_result and execution_result.get("response") is not None:
                    self.memory_manager.store_goal_memory(
    ...
    def _determine_task_type_and_confidence(self, task_prompt: Dict[str, Any]) -> Tuple[str, str]:
        ...
        """
        try:
            # Extract text for analysis for heuristic approach
            analysis_text = ""
            if 'components' in task_prompt and isinstance(task_prompt['components'], dict):
                components = task_prompt['components']
                analysis_text = f"{components.get('task', '')} {components.get('purpose', '')} {components.get('context', '')}".strip()
            ...
            # If not directly provided or one is missing, use heuristic analysis
            if not task_type or not confidence_level:
                if not analysis_text: # Ensure analysis_text has some content for heuristic
                    # Fallback for analysis_text using name or string representation of prompt
                    analysis_text = task_prompt.get('name', str(task_prompt))
                
                logger.debug(f"Task type/confidence not explicit or partially missing. Using heuristic analysis on: '{analysis_text[:100]}...'" )
